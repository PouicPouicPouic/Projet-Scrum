        ...
        if arg4 != 'all' : #filtre les fichiers à convertir selon ce qui est indiqué par arg4
            if len(arg4) > 1 :
                ...
                while n < len(arg4)-1 : #indique le nombre d'éléments à analyser (si on considère qu'il n'y a pas de doublons dans arg4)
                    filtre = []
                    int_arg4 = int(arg4[n])
                    filtre.append(textes[int_arg4]) #fonctionne si arg4 est considéré comme un int, pas si il est considéré comme une str /!\
                    textes = filtre
                    n = n+1
            else :
                n = 0
                while n < len(arg4) : #indique le nombre d'éléments à analyser (si on considère qu'il n'y a pas de doublons dans arg4)
                    filtre = []
                    int_arg4 = int(arg4[n])
                    filtre.append(textes[int_arg4]) #fonctionne si arg4 est considéré comme un int, pas si il est considéré comme une str /!\
                    textes = filtre
